...
> **Amendment (2026-05-27, broadened 2026-05-29, expanded 2026-05-29):** On import, a collection
> is seeded with a **curiosity-driven starter wiki** generated by the LLM from the papers'
> abstracts AND (always, where cached) PDF excerpts — a field overview (hook, one-sentence /
> one-paragraph, core tension, why-it-matters), problem / method / open-problem / benchmark
> landscapes, **per-paper cards** (problem / idea / method-family / contribution / why-read /
> status / difficulty / prerequisites / connected-papers and, for papers with PDF excerpts only,
> mechanism / evidence / limitation), and **reading paths** (plural, ≥3 papers per path or
> dropped). Meant to orient and spark reading, not summarize. Produced by the `starter-wiki`
> skill (`app/skills/starter-wiki/SKILL.md`), stored as structured `wiki/overview.json`, rendered
> as an interactive multi-tab HTML view. This is a deliberate, narrow exception to "wiki = the
> user's thinking" and "only an accepted proposed edit writes the wiki." Constraints that hold:
> it's **non-destructive** (separate from the notes-based markdown wiki; only `overview.json` is
> written; regenerate is explicit), **honestly tagged** as machine-made (`_meta.generated_by:
> agent` + in-view banner) — paper cards from abstracts-only papers carry an `abstract_only`
> flag and have mechanism/evidence/limitation blanked; reading paths are **dropped** rather than
> faked when the collection lacks ≥3 fitting papers; refs are **validated** against the
> collection (hallucinated citations and unsupported sections are dropped, never invented). See
> `wiki.generate_overview` / `wiki.load_overview`. The notes-based wiki (chat-derived edits,
> notes, the curator-driven refresh) still goes through the propose→review→accept gate.
> Features that mint claims about the user's mind or unverified inter-paper debates remain
> review-queue-gated and outside this exception.
