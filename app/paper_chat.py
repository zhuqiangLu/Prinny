...
def _system(collection: str, title: str, paper_id: int, pdf_path: str, n_prior: int = 0) -> str:
    return (
        ...
        "CROSS-PAPER (contradiction / connection / similarity with OTHER papers the user has "
        "read): find candidates with list_papers + search_fragments (it covers their notes, "
        "thoughts, AND highlights), then call get_paper_context(other_paper_id) to read the "
        "user's notes/highlights on that paper — prefer their own take; use "
        "read_paper_text(other_paper_id) only if their notes are thin. CITE the specific "
        "passages (page / fragment) behind any contradiction or link you claim; never "
        "manufacture one to be helpful.\n"
...
def _codex_system(collection: str, title: str, paper_id: int, n_prior: int = 0) -> str:
    return (
        ...
        "the user for the paper list. For contradiction/connection/similarity with OTHER "
        "papers, find candidates via list_papers + search_fragments (covers notes, thoughts, "
        "highlights), then get_paper_context(other_paper_id) to read the user's take on them, "
        "and cite the specific passages. You have READ-only access; do not "
